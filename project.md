...
|----|--------|-------------|-------|
| #72 | `fix/config-concurrency-rmw` | **Updated (open)** | v5.51.0 release PR. Code Rabbit findings addressed in-branch (aa0cfb0, 4ab2975); polls channel-guard localized per review. B-018 fixed; B-019 sweep verified benign (closed, docs-only); B-008 confirmed fixed (pyright clean 2026-07-02). Merged `origin/main` (v5.50.2 + dependabot action bumps) to clear the CONFLICTING state; CLAUDE.md/AGENTS.md action-pin invariant reconciled to `checkout@v7`. CodeQL alerts triaged as false positives (cyclic-import = `_MixinBase` TYPE_CHECKING architecture; economy multiple-definition = deliberate seeds); 3 dead imports removed (71363ed). **Ten auto-applied CodeQL autofix commits reverted (d01715a)** — they broke the blocking ruff gate (F821 on TYPE_CHECKING string annotations), gutted `_BotBase` typing, and crashed a levels test (`int.__module__`); do not re-apply. Benchmark check flaked once on `overhead_percent` (34% vs 30% hard limit, byte-identical code) — passed on rerun. |
